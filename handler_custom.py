...
try:
    import psycopg2
    print(sys.modules['psycopg2'].__file__)
except ImportError:
...
class LambdaHandler:
    ...
    # def run_cron(self):
    #     """Runs the Django management command 'cron'."""
    #     from django.core import management
    #     management.call_command('cron')

    def run_cron(self):
        """Runs the Django cron command with a patched Queue."""
        import sys
        from django.core import management
        from queue import Queue as ThreadQueue

        # Monkey patch multiprocessing.Queue with queue.Queue before importing chroniker
        # Only do this if multiprocessing module exists
        if 'multiprocessing' not in sys.modules:
            import multiprocessing
        sys.modules['multiprocessing'].Queue = ThreadQueue
        
        try:
            management.call_command('cron')
            return {"message": "Cron jobs completed successfully"}
        except Exception as e:
            print(f"Error running cron command: {str(e)}")
            return {"error": str(e)}

    # def run_cron(self):
    #     """Runs cron jobs directly without using the chroniker management command."""
    ...
    #     try:
    #         now = timezone.now()
    #         print(f'Running cron jobs at {now}')
            
    #         jobs = Job.objects.due().filter(enabled=True)
    #         print(f'Found {jobs.count()} jobs to run')
    ...
    #                 if job.is_running:
    #                     print(f'Job "{job.name}" already running. Skipping.')
    #                     continue
    ...
    #                 job.last_run_successful = True
    #                 job.last_run_end_timestamp = timezone.now()
                    
    #                 # Calculate next run based on frequency
    #                 if job.frequency:
    #                     try:
    #                         frequency_minutes = int(job.frequency)  # Convert string to integer
    #                         job.next_run = job.last_run_start_timestamp + timedelta(minutes=frequency_minutes)
    #                     except (ValueError, TypeError):
    #                         print(f"Invalid frequency value for job {job.name}: {job.frequency}")
    #                         job.next_run = None
    #                 else:
    #                     job.next_run = None
    ...
    #                 print(f'Successfully completed job: {job.name}')
    #             except Exception as e:
    ...
    #                 print(f'Error running job {job.name}: {str(e)}')
    #                 logger.exception(f'Error in cron job {job.name}')
    #             finally:
    #                 # Always ensure we mark the job as not running
    #                 job.is_running = False
    #                 job.save()
                    
    #         return {"message": "Cron jobs completed", "jobs_processed": jobs.count()}
    #     except Exception as e:
    #         logger.exception("Error in run_cron")
    #         return {"error": str(e)}
...
def run_cron_handler(event, context):
    print('running function run_cron_handler')
    handler = LambdaHandler()
    return handler.run_cron()